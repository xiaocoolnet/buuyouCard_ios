//
//  LoseOrderTableViewController.swift
//  BuuYou
//
...
class LoseOrderTableViewController: UITableViewController {
    
    var LoseSource = LoseList()
    ...
    func Yanzheng(){
        ...
        //let str = "QQ卡"
        let params = [
            "data":"\(Account!),\(Password!),\(DateValue),\(Date),\(OrNumber!),\(intValue),,\(Number!),2,1,100,1"
        ]
        Alamofire.request(.GET, url, parameters: params).response { request, response, json, error in
            if(error != nil){
                ...
                if(status.status == 1){
                    ...
                    self.LoseSource = LoseList(status.data!)
    ...
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return LoseSource.count
    }
    
    override  func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier("LoseCell", forIndexPath: indexPath) as!LoseOrderTableViewCell
        let orderInfo = self.LoseSource.objectlist[indexPath.row]
        let dateformate = NSDateFormatter()
        dateformate.dateFormat = "MM-dd"
        cell.CardName.text = orderInfo.ChannelName!
        cell.CardNumber.text = orderInfo.BillNO!
        cell.Time.text = orderInfo.PayDate!
        //强制类型转换
        cell.Money.text = String(orderInfo.OrderMoney!)
        return cell
    }
