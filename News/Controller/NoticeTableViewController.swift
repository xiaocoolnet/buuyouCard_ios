...
class NoticeTableViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.NoticeTitle.text = noticeInfo.title
        cell.NoticeDate.text = noticeInfo.datatime
        //cell.NoticeImages.image = UIImage(named: "通知公告图片")
        //用户头像的获取
        let imgUrl = NoticeImageUrl+(noticeInfo.picpath!)
        ...
        NSURLConnection.sendAsynchronousRequest(request, queue: NSOperationQueue.mainQueue(), completionHandler: {(response: NSURLResponse?,data: NSData?,error: NSError?)-> Void in
            if(data != nil){
                ...
                cell.NoticeImages.image = imgTmp
                cell.NoticeImages.alpha = 1.0
